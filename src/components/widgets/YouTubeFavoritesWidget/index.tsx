...
const parseYouTubeUploadsFeed = (xml: string, channelId: string): YouTubeFavoriteVideo[] => {
  ...
  return entries
    .map((entry): YouTubeFavoriteVideo | null => {
      ...
      const channelTitle = entry.querySelector('author > name')?.textContent?.trim() || feedTitle;
...
const YouTubeFavoritesWidget: React.FC<YouTubeFavoritesWidgetProps> = ({ config }) => {
      ...
      >
        ...
        <div className="min-w-0 flex-1 flex flex-col justify-between">
          <div className="min-w-0">
            <div className="text-sm font-medium text-gray-900 dark:text-gray-100 line-clamp-2">{video.title}</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">
              {video.channelTitle ?? video.channelId}
            </div>
          </div>
          {video.publishedAt && (
            <div className="text-xs text-gray-500 dark:text-gray-400">{formatDate(video.publishedAt)}</div>
          )}
        </div>
      </a>
    );
            ...
            {channelIds.map((id) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3">
                    <AutoScrollContainer enabled={!!localConfig.autoScroll} className="w-full" style={{ height: viewportHeightPx }}>
                      <div className="space-y-2">{items.map(renderVideoRow)}</div>
                    </AutoScrollContainer>
                  </div>
                </div>
              );
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-2">
                ...
                {(localConfig.channelIds ?? []).map((id, idx) => (
                  <div key={idx} className="flex gap-2">
                    <Input value={id} onChange={(e) => updateChannelId(idx, e.target.value)} placeholder="Channel ID" />
                    <Button type="button" variant="outline" size="icon" onClick={() => removeChannelId(idx)} aria-label="Remove channel">
                      <Trash2 size={16} />
                    </Button>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setLocalConfig({ ...localConfig, videosToDisplay: Number(e.target.value || 1) })}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setLocalConfig({ ...localConfig, videosToList: Number(e.target.value || 1) })}
                />
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="display" className="space-y-4 py-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch checked={!!localConfig.autoScroll} onCheckedChange={(checked) => setLocalConfig({ ...localConfig, autoScroll: checked })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <div className="text-xs text-gray-500 dark:text-gray-400">Set to 0 to disable auto-refresh.</div>
            </div>
          </TabsContent>
        </Tabs>
  ...
  return (
    <div ref={widgetRef} className="widget-container h-full flex flex-col relative">
      <WidgetHeader
        title={localConfig.title || defaultConfig.title || 'YouTube Favorites'}
